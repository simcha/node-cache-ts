...
export default class NodeCache<VT> extends EventEmitter {
        ...
        data: Data<VT> = {};
        stats: { hits: number; misses: number; keys: number; ksize: number; vsize: number; };
		...
		constructor( options: Options = {} ){
			...
			// 		"keys",
			// 		"has"
			// 	].forEach((methodKey: string | number) => {
			// 		// reference real function
			// 		const oldMethod = this[methodKey];
			// 		this[methodKey] = function(...args1: { length?: any; slice?: any; }) {
			// 			// return a callback if cb is defined and a function
			// 			const adjustedLength = Math.max(args1.length, 1), args = args1.slice(0, adjustedLength - 1), cb = args1[adjustedLength - 1];
			// 			if (typeof cb === "function") {
			// 				try {
			// 					const res = oldMethod(...Array.from(args || []));
			// 					cb(null, res);
			// 				} catch (err) {
			// 					cb(err);
			// 				}
			// 			} else {
			// 				return oldMethod(...Array.from(args), cb);
			// 			}
			// 		};
			// 	});
			//}
		...
		mget( keys: (Key)[] ){
			...
			if (!Array.isArray( keys )) {
				const _err = this._error("EKEYSTYPE", "The keys argument has to be an array.");
				throw _err;
			}
		...
		mset( keyValueSet: { key: number | string, val: VT, ttl: number }[] ) {
			// check if cache is overflowing
			let _err: Error, key: number | string, keyValuePair: { key: number | string; val: VT; ttl: number; }, ttl: number, val: VT;
		...
		_isInvalidKey( key: unknown ){
			if (!Array.from(this.validKeyTypes).includes(typeof key)) {
				return this._error("EKEYTYPE", `The key argument has to be of type \`string\` or \`number\`. Found: \`${typeof key}\``);
			}
		}
		...
		_wrap( value: VT | string, ttl: number | string, asClone: boolean = true): WrappedValue<VT>{
		...
		_getValLength( value: VT | string){
			if (typeof value === "string") {
				...
				return 8;
			} else if (value && typeof value === "object" && typeof value.hasOwnProperty('then') === 'function') {
