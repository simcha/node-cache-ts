 ...
 */
export class SelfRefreshingCache<A extends {[key: string]: Serializable}, T> extends NodeCache<T> {
    ...
    constructor(options: RefreshOptions, refreshMethod: (args: A) => Promise<T> ) {
        ...
        this._refreshArgsCache = new NodeCache<A>({stdTTL: this.ttr, checkperiod: options.checkperiod}).on( 'expired', ( key, value ) => {
            this.refreshEntry(key, value);
        });
        this.refreshMethod = refreshMethod;
    }

    public async getSet(key: Key, args: A): Promise<T> {
        let response:T;
        if (super.has(key)){
    ...
    private async refreshEntry(key: string, value: A) {
        try {
            const response = await this.refreshMethod(value);
            super.set(key, response);
        } catch (error) {
            const ttl = super.getTtl(key);
            if (ttl && this.retryPause*1000 < ttl) {
                this._refreshArgsCache.set(key, value, this.retryPause);
            } 
